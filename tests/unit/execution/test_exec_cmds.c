...
static int test_builtin_dup_fails_redir_in(void)
{
    ...
    int status = exec_cmds(sh, cmds);
    wait_all_children();
    logger_open_fds("♻️test_builtin_dup_fails_redir_in♻️", "should be restored");
...
static int test_external_fork_fails_redir_in(void)
{
    ...
    int status = exec_cmds(sh, cmds);
    wait_all_children();
    logger_open_fds("♻️test_external_fork_fails_redir_in♻️", "should be restored");
    fd_teardown_all_mocks();
...
static int test_external_fork_fails_redir_out(void)
{
    ...
    int status = exec_cmds(sh, cmds);
    wait_all_children();
    logger_open_fds("♻️test_external_fork_fails_redir_out♻️", "should be restored");
    fd_teardown_all_mocks();
...
static int test_external_fork_fails_redir_in_out(void)
{
    ...
    fd_setup_fork_fail_at(0);
    logger_open_fds("♻️test_external_fork_fails_redir_in_out♻️", "should be restored");
    int status = exec_cmds(sh, cmds);
    wait_all_children();
...
static int test_pipeline_builtin_fork_fails_first(void)
{
    ...
    int status = exec_cmds(sh, cmds);
    wait_all_children();
    logger_open_fds("♻️test_pipeline_builtin_fork_fails_first♻️", "should be restored");
    fd_teardown_all_mocks();
...
{
    printf("Test: builtin_in_out open fails | builtin | builtin\n");
    t_shell *sh = create_test_shell(test_env, 0);
...
{
    printf("Test: external_in | external | external_out (all ok)\n");
    t_shell *sh = create_test_shell(test_env, 0);
...
int main(void)
{
    ...
    // mu_run_test(test_pipeline_builtin_fork_fails_second);
    // mu_run_test(test_pipeline_builtin_fork_fails_third);

    // printf("\n--- Open fails ---\n");
    // mu_run_test(test_pipeline_builtin_open_fails_first_in);
    // mu_run_test(test_pipeline_builtin_open_fails_first_out);
    // mu_run_test(test_pipeline_builtin_open_fails_first_in_out);

    printf("\n--- Dup2 fails ---\n");
    // VOY POR AQUÍ, cuando falla el dup2 del último comando sale con un pipe de más
    // mu_run_test(test_pipeline_builtin_dup2_fails_middle);
    ...
    // mu_run_test(test_pipeline_external_fork_fails_second);
    // mu_run_test(test_pipeline_external_fork_fails_third);
    
    // printf("\n--- Open fails ---\n");
    // mu_run_test(test_pipeline_external_open_fails_first_in);
    // mu_run_test(test_pipeline_external_open_fails_second_in);
    // mu_run_test(test_pipeline_external_open_fails_third_in);
