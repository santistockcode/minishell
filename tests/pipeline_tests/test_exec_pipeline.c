...
TestSuite(exec_pipeline);

Test(exec_pipeline, redirs_in_and_out_append_returns_zero)
{
    ...
    int rc = exec_pipeline(sh, pipe_head);
    cr_assert_eq(rc, 0, "exec_pipeline should return 0 for valid pipeline");

    free_pipeline(pipe_head);
    free(sh);
}

Test(exec_pipeline, empty_pipeline_returns_zero)
{
    t_shell *sh = (t_shell*)malloc(sizeof(t_shell));
    cr_assert_neq(sh, NULL, "malloc shell failed");
    sh->i = 0;
    ...
    t_list *pipe_head = NULL;
    int rc = exec_pipeline(sh, pipe_head);
    cr_assert_eq(rc, 0, "exec_pipeline should return 0 for empty pipeline");
    free(sh);
}
