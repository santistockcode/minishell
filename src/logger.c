...
/*   Updated: 2026/02/09 21:22:17 by saalarco         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
typedef struct s_shell
{
	...
	t_list		*pars_cmds; // Parsing cmds,
				to be freed afte each readline iteration
...
*/
void	logger_minishell_struct(t_shell *sh, const char *tag,
		const char *message)
{
	(void) sh;
	(void) tag;
...
// 			tag, sh->last_status, sh->last_err_op ? sh->last_err_op : "none",
// 			message);
// 	}
// 	printf(COLOR_RESET);
// 	if (LOG == 1)
...
// 	printf(COLOR_CYAN);
// 	printf("%s [%s] (last_status: %d, last_err_op: %s): %s\n", time_str,
// 		tag ? tag : "(no tag)", sh ? sh->last_status : -1, (sh
// 			&& sh->last_err_op) ? sh->last_err_op : "none",
...
// 				redir->target ? redir->target : "(null)", redir->quoted);
// 		}
// 		redir_node = redir_node->next;
// 	}
// 	printf(COLOR_RESET);
...
// 				token->value ? token->value : "(null)", token->syntax_error);
// 		}
// 		node = node->next;
// 	}
// }
...
// 		printf("  cmd[%d] word=\"%s\"\n", index, cmd
// 			&& cmd->cmd_word ? cmd->cmd_word : "(null)");
// 		if (cmd)
// 		{
// 			sub = cmd->cmd_prefix;
// 			while (sub)
...
// 			snprintf(path, sizeof(path), "%s%.*s", fd_prefix, (int)avail,
// 				entry->d_name);
