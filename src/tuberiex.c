    ...
    const t_stage_io *io, t_list *env, int *p)
{
    ...
    if (!path)
    {
        ...
        return (sh->last_status = 127, -1); // pending testing
    }
...
int do_last_command(t_shell *sh, t_cmd *cmd, int last_fd)
{
    ...
    {
        redirs = cmd->redirs;
        logger("do_last_command", "Preparing redirections, I'm in child");
        logger("do_last_command:last_fd", ft_itoa(last_fd));
        if (prepare_redirs(redirs) == -1)
            return (safe_close_redirs(redirs), -1);
        rdr_spec.in_fd = last_fd;
        rdr_spec.out_fd = get_r_out_redir_fd(redirs);
        rdr_spec.out_mode = get_r_out_mode(redirs);
        logger("do_last_command:rdr_spec:in_fd", ft_itoa(rdr_spec.in_fd));
        logger("do_last_command:rdr_spec:out_fd", ft_itoa(rdr_spec.out_fd));
        logger("do_last_command:rdr_spec:out_mode", ft_itoa(rdr_spec.out_mode));
        if (msh_exec_stage(sh, cmd, &rdr_spec, sh->env, p) == (-1))
            return (safe_close_redirs(redirs), -1);
    }
    safe_close(last_fd);
...
int do_first_command(t_shell *sh, t_cmd *cmd, int *p)
{
    ...
    if (pid == 0)
    {
        ...
        rdr_spec.out_fd = p[1];
        rdr_spec.out_mode = 0;
        logger("do_first_command:rdr_spec:in_fd", ft_itoa(rdr_spec.in_fd));
        logger("do_first_command:rdr_spec:out_fd", ft_itoa(rdr_spec.out_fd));
        logger("do_first_command:rdr_spec:out_mode", ft_itoa(rdr_spec.out_mode));
        logger("do_first_command", "Executing first command stage");
        if (msh_exec_stage(sh, cmd, &rdr_spec, sh->env, p) == (-1))
            return (safe_close_redirs(redirs), -1);
    }
    safe_close(p[1]);
...
int run_pipeline(t_shell *sh, t_list *cmd_first, int nstages)
{
    ...
    if (nstages <= 1)
    {
        logger("run_pipeline", "Incorrec flow trying to run pipeine with 1 or less stages");        
        return (-1);
    }
    if (pipe_wrap(p) == -1)
        return (msh_set_error(sh, PIPE_OP), -1);
    logger("run_pipeline", "Pipe created for first command");
    logger("run_pipeline:nstages", ft_itoa(nstages));
    current_cmd_node = cmd_first;
    response = do_first_command(sh, (t_cmd *)current_cmd_node->content, p);
    if (response == -1)
        return (-1);
    logger("run_pipeline", "First command executed, exchange pipe");
    in_fd = p[0];
    ...
    while (i < nstages - 1)
    {
        if (pipe_wrap(p) == -1)
            return (msh_set_error(sh, PIPE_OP), -1);
        logger("run_pipeline", "Pipe created for middle command");
        response = do_middle_commands(sh, (t_cmd *)current_cmd_node->content, p, in_fd);
        if (response == -1)
            return (-1);
        logger("run_pipeline", "Middle command executed, exchange pipe");
        in_fd = p[0];
        current_cmd_node = current_cmd_node->next;
        i++;
    }
    logger("run_pipeline", "Middle commands executed or inexistant");

    response = do_last_command(sh, (t_cmd *)current_cmd_node->content, in_fd);
    if (response == -1)
        return (-1);
    logger("run_pipeline", "Last command executed");
    return (response);
}
