...
/*   Updated: 2026/02/09 22:04:12 by saalarco         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	free_main_loop(t_shell *minishell, int *status)
{
	if (minishell->lexing)
...
- En caso de que le usuarie introduzca 'echo "hello"',
	compruebo en el logger 
	de la parte de ejecución que args[1] es hello y no 
	"hello" como sucede ahora.
...
- Después de entrar 1 vez en el bucle, minishell debe quedar tal que:
	...
	t_list		*cmds_start;
// NULL (tanto antes de llamar a exec_cmds como a la salida debe ser NULL)
	...
	char		*last_err_op;
				// seguir siendo null dado que no ha habido un error
	int			last_errno;	// 0 dado que no ha habido un error
	int			save_in; // 0
	int			save_out; // 0
	int			save_err; // 0

- Después de entrar en el bucle, hacer control+C y salir,
	minishell debe quedar tal que:

- Después de entrar en el bucle, hacer control+D y salir,
	minishell debe quedar tal que:

- Después de interrumpir el tratado de here_docs y volver al bucle,
	minishell debe quedar tal que:
...
- Comandos con argumento entrecomillado,
	este argumento no llega a la parte de ejecución.
...
SANTI:
	...
	- hay un bug (o diferencia con bash, cuando se ejecuta algo con un slash,
		como ./command,
		no es capaz de ejecutar un script 
		./whatever.sh) ni diferencia en caso de error
...
int	main(int argc, char **argv, char **envp)
{
	...
	while (minishell->should_exit == 0)
	{
		...
		free_main_loop(minishell, &status);
	}
